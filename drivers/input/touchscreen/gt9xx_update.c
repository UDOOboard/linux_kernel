...
s32 gup_enter_update_mode(struct i2c_client *client)
{
    ...
    GTP_GPIO_OUTPUT(gpio_reset_port, 0);
    msleep(2);
...
void gup_leave_update_mode(s32 irq_gpio_port)
{
    GTP_GPIO_AS_INT(irq_gpio_port);
...
update_fail:
    ...
    gup_leave_update_mode(ts->int_gpio);
...
s32 gup_clk_calibration(s32 gpio_irq_port)
{
    u8 buf;
    //u8 trigger;
    s32 i;
