...
/*
 * The code contained here in is licensed under the GNU General Public
		...
		const char *buf, size_t count)
{
	...
	if (can_test_en) {
		...
		pr_info("\ncan test A9 mcc prepares run, MCC version is %s\n",
		...
		while (can_test_en) {
			...
			if (ret >= 0) {
//pr_info("recBytes=%d", num_of_received_bytes);
				pr_info("M4 log: %s", msg.data);
			}
		}
/*
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (pingpong_en) {
		...
		pr_info("\nping pong A9 mcc prepares run, MCC version is %s\n",
				mcc_info.version_string);
		msg.data = 1;
		while (i < 0x50000) {
			i++;
...
static int imx_mcc_test_probe(struct platform_device *pdev)
{
	...
	pr_info("M4 debugger for uDooNeo prepares run, MCC version is %s\n",
			mcc_info.version_string);
...
static const struct of_device_id imx_mcc_test_dt_ids[] = {
	{ .compatible = "fsl,imx6sx-m4-debugger", },
	{ /* sentinel */ }
};
...
static int __init imx_mcc_test_init(void)
{
	...
	if (ret)
		...
		pr_info("imx M4 debugger for uDooNeo is registered.\n");
	return ret;
}
late_initcall(imx_mcc_test_init);

MODULE_AUTHOR("Seco S.r.L.");
MODULE_DESCRIPTION("IMX M4 UDOONEO DEBUGGER, BASED ON MCC");
MODULE_LICENSE("GPL");
